...
namespace WoorkNeedles.Controller.Productos
{
    ...
    ) : ControllerBase
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] UpdateTallaDto dto)
        {
            try
            {
                await update.Execute(id, dto);
                return Ok(new { message = "Talla actualizada exitosamente." });
            }
            catch (KeyNotFoundException ex) { return NotFound(ex.Message); }
            catch (DomainException ex) { return BadRequest(ex.Message); }
        }
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                return Ok(new { message = "Talla eliminada exitosamente." });
            }
            catch (KeyNotFoundException ex) { return NotFound(ex.Message); }
        }
    }
